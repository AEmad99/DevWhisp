...
<div class="overlay" role="dialog" aria-modal="true" aria-label="DevWhisp setup">
  <div class="wizard">
    ...
    <div class="panel">
      {#if step === 0}
        <span class="hero-icon"><AppIcon size={84} /></span>
      ...
      {:else if step === 2}
        ...
        <p>
          Download a small on-device model (Tiny recommended). It runs fully offline on your CPU. One-time download (~30-75 MB).
        </p>
...
<style>
  ...
  .panel { min-height: 230px; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .hero-icon { filter: drop-shadow(0 8px 24px rgba(124, 58, 237, 0.5)); margin-bottom: 18px; }
  .step-glyph { font-size: 44px; margin-bottom: 14px; filter: drop-shadow(0 4px 14px rgba(124, 58, 237, 0.35)); }

  h2 { margin: 0 0 10px; font-size: 22px; font-weight: 700; letter-spacing: -0.02em; color: var(--text); }
  p { margin: 0 0 10px; font-size: 14px; line-height: 1.55; color: var(--muted); max-width: 360px; }
  ...
  .status.ready { color: var(--ok); border-color: rgba(92, 255, 156, 0.3); }
  ...
  .actions .primary { background: var(--brand-gradient); border: none; color: #fff; box-shadow: var(--shadow-accent); }
